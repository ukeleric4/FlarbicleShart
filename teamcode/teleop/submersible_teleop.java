...
//    }
    ...
    public void slidePos() {
        if (gamepad1.x) {
            ...
            while (getRuntime() - runtime < 0.75) {follower.update();}
            panningMotor.stop();
            ...
            panningMotor.runDown();
            double runtime = getRuntime();
            while (getRuntime() - runtime < 0.75) {follower.update(); slides.updateSlide();}
            panningMotor.stop();
        } else if (gamepad1.y) {
    ...
    public void clawMovement() {
        if (gamepad2.dpad_down) {
            pitching.moveToMax();
            double runtime = getRuntime();
            while (getRuntime() - runtime < 0.3) {follower.update();}
            claw.closeClaw();
            double runtime2 = getRuntime();
            while (getRuntime() - runtime2 < 0.6) {follower.update();}
            pitching.moveToMin();
            ...
            slides.setTargetPos(1000);
            while (slides.getCurrentPos() < 950) {slides.updateSlide(); follower.update();}
            double runtime = getRuntime();
            while (getRuntime() - runtime < 0.3) {follower.update();}
            claw.openClaw();
            slides.setTargetPos(30);
            while (slides.getCurrentPos() > 30) {slides.updateSlide(); follower.update();}
            panningServo.moveSpecificPos(0.3);
        }
    }
